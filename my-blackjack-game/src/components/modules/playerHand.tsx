...
/**
 * 
 * @param props a hand which includes information about the player who owns it, the cards it hold, the betsize,
 * and the handvalue, whether or not the hand is the current hand in play, and a result representing if the 
 * hand beats the dealer, which is only meaningful during the reveal phase
 * @returns 
 */
const Hand = (props: { 
  hand: displayHand,
  ...
  //restructing hand to contain the suite symbol instead of char, 
  // if a card is given as hidden, will show a flipped card sintead
  hand.cards.forEach(card=>{
    let newCard = card;
    if(card !== "HIDDEN") newCard = (card.substring(0, card.length - 1) + suiteMapping.get(card[card.length - 1]));
    cards.push(newCard);
  })
  

  return (
    //Determines the color of the container depending on result status
    <div className={`flex flex-col items-center gap-2 md:gap-3 p-2 md:p-3 rounded-lg border-2 relative transition-all duration-200 min-w-[100px] max-w-[140px] ${
      ...
      {/* Card Info : Determines whether card front or back is shown*/}
      <div className="relative flex items-center">
        {cards.map((card, i) => (
          card === "HIDDEN" ? <div className='w-14 h-20 flex items-center justify-center rounded-lg border bg-white font-bold shadow-md text-5xl'>
            🂠
          </div>:
      ...
      {/* Player Info - Displays the player name, and betSize*/}
      <div className='text-center w-full'>
        ...
        {/* Result Text - Displays the result, whether or not hand won, or tied. */}
        { result === "W" && (
